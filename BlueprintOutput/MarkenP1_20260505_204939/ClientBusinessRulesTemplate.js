function ClientBusinessRules() {
    ...
    this.NewShipment = function(shipmentRequest) {
        ...
        */
        var shipment = shipmentRequest || this.vm.shipmentRequest; // Accept the passed object or fall back to the ViewModel.
        if (!shipment) {
        ...
        if (!shipment.PackageDefaults) {
            shipment.PackageDefaults = {}; // Create the container when the template has not supplied one yet.
        }
        ...
        if (userAddress) {
            ...
            shipment.PackageDefaults.Consignee = shipment.PackageDefaults.Consignee || {}; // Preserve any existing object if the template already created one.
            shipment.PackageDefaults.Consignee.Company = userAddress.Company || ''; // Use user address company for Pickup From.
            shipment.PackageDefaults.Consignee.Contact = userAddress.Contact || ''; // Preserve contact details for return label pickup origin.
            shipment.PackageDefaults.Consignee.Address1 = userAddress.Address1 || '';
            shipment.PackageDefaults.Consignee.Address2 = userAddress.Address2 || '';
            shipment.PackageDefaults.Consignee.Address3 = userAddress.Address3 || '';
            shipment.PackageDefaults.Consignee.City = userAddress.City || '';
            shipment.PackageDefaults.Consignee.StateProvince = userAddress.StateProvince || '';
            shipment.PackageDefaults.Consignee.PostalCode = userAddress.PostalCode || '';
            shipment.PackageDefaults.Consignee.Country = userAddress.Country || ''; // This country drives Canada vs. non-Canada UI logic.
            shipment.PackageDefaults.Consignee.Phone = userAddress.Phone || '';
        }
        ...
        shipment.PackageDefaults.ShipperReference = getCustomValue('Custom2');
        // Protocol Number comes from user Custom1 and is stored in MiscReference1.
        shipment.PackageDefaults.MiscReference1 = getCustomValue('Custom1');
        // Site Number comes from user Custom3 and is stored in MiscReference2.
        shipment.PackageDefaults.MiscReference2 = getCustomValue('Custom3');

        // Step 5: Apply the blueprint defaults for specimen-return shipping.
        shipment.PackageDefaults.Description = shipment.PackageDefaults.Description || 'UN3373 Category B Human Sample'; // Default description from the blueprint.
        shipment.PackageDefaults.ReturnDelivery = true; // Return delivery must be on for the specimen-return workflow.
        shipment.PackageDefaults.SaturdayDelivery = true; // Saturday delivery defaults on per blueprint.
        shipment.PackageDefaults.Terms = shipment.PackageDefaults.Terms || 'Prepaid'; // Default terms to prepaid.

        // Step 6: Set the default service to UPS Express when none is already assigned.
        if (!shipment.PackageDefaults.Service) {
            shipment.PackageDefaults.Service = {}; // Create a service object so the UI has something to display.
        }
        if (!shipment.PackageDefaults.Service.Symbol) {
            shipment.PackageDefaults.Service.Symbol = 'CONNECTSHIP_UPS.UPS.EXP'; // Blueprint default service.
            shipment.PackageDefaults.Service.Name = 'UPS Express'; // Friendly label for the selected service.
        }
        ...
        if (shipment.Packages && shipment.Packages.length > 0) {
            if (!shipment.Packages[0].Weight) {
                shipment.Packages[0].Weight = {}; // Build the weight object if the template has not done so.
            }
            shipment.Packages[0].Weight.Units = shipment.Packages[0].Weight.Units || 'KG'; // EU/UK profile default from the blueprint.
        }
    };
    ...
    this.Keystroke = function(shipmentRequest, vm, event) {
        ...
        */
        var shipment = shipmentRequest || this.vm.shipmentRequest; // Use the active shipment from the ViewModel.
        var eventObject = event || window.event; // Support browser event naming differences.
        if (!shipment || !eventObject) {
        ...
        var tempValue = '';
        if (shipment.PackageDefaults && shipment.PackageDefaults.ConsigneeReference) {
            tempValue = (shipment.PackageDefaults.ConsigneeReference || '').toString();
        }
        ...
        if (defaultWeight !== null && shipment.Packages && shipment.Packages.length > 0) {
            if (!shipment.Packages[0].Weight) {
                shipment.Packages[0].Weight = {}; // Create the Weight object if it does not already exist.
            }
            shipment.Packages[0].Weight.Amount = defaultWeight; // Populate the package weight to match the selected temperature category.
            shipment.Packages[0].Weight.Units = shipment.Packages[0].Weight.Units || 'KG'; // Keep the default unit aligned with the blueprint.
        }
    ...
    this.PostLoad = function(loadValue, shipmentRequest) {
        ...
        */
        var shipment = shipmentRequest || this.vm.shipmentRequest; // Use the loaded shipment from the view model.
        if (!shipment) {
        ...
        if (defaultWeight !== null && shipment.Packages && shipment.Packages.length > 0) {
            if (!shipment.Packages[0].Weight) {
                shipment.Packages[0].Weight = {}; // Create the weight object when needed.
            }
            shipment.Packages[0].Weight.Amount = defaultWeight; // Populate the UI weight field with the temperature-specific default.
            shipment.Packages[0].Weight.Units = shipment.Packages[0].Weight.Units || 'KG'; // Preserve the blueprint's kilogram default.
        }
    ...
    this.PreShip = function(shipmentRequest, userParams) {
        ...
        */
        var shipment = shipmentRequest || this.vm.shipmentRequest; // Use the current shipment object.
        if (!shipment) {
        ...
        if (tempValue === 'Frozen') {
            ...
            if (!dryIceValue && shipment.PackageDefaults && shipment.PackageDefaults.MiscReference3) {
                dryIceValue = (shipment.PackageDefaults.MiscReference3 || '').toString().trim();
            }
    ...
    this.PostBuildShipment = function(shipmentRequest) {
        ...
        */
        var shipment = shipmentRequest || this.vm.shipmentRequest; // Use the collected shipment object.
        if (!shipment) {
        ...
        if (!shipment.PackageDefaults) {
            shipment.PackageDefaults = {}; // Create the shipment default container if the template omitted it.
        }
        ...
        if (dryIceInput && (dryIceInput.value || '').trim()) {
            shipment.PackageDefaults.MiscReference3 = (dryIceInput.value || '').trim(); // The server expects this exact field for dry ice processing.
        }
        ...
        if (biologicalInput) {
            shipment.PackageDefaults.MiscReference4 = biologicalInput.checked ? 'true' : 'false'; // Preserve the checkbox state as a boolean-style string.
        }
...
/* CBR hooks required by the blueprint:
   - PageLoaded: redirect to shipping page, hide Pickup button for Canada
   - NewShipment: copy user address/custom data into Pickup From and defaults
   - PostLoad: set package weight from Temperature and toggle dry-ice editability
   - Keystroke: react to Temperature changes
   - PreShip: require/stash Dry Ice Weight when Frozen
   - PostBuildShipment: normalize MiscReference3 and Biological Sample values

   The blueprint’s UI behavior is template-driven, but the detailed interaction logic should be implemented in the existing CBR file. */
}