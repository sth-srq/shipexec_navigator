...
namespace PSI.Sox
{
    ...
    public class SoxBusinessRules : IBusinessObject
    {
        ...
        public void PreShip(ShipmentRequest shipmentRequest, SerializableDictionary userParams)
        {
            ...
            // This hook is the authoritative enforcement point for the specimen-return rules.
            // Blueprint Phase 1 SBR PreShip: paperless invoice, export reason, CS adapter preference,
            // rate-shop-based service correction, biological sample flags, and dry ice conversion.
            // We delegate the heavy business logic to a manager class so SoxBusinessRules stays thin.
            // ─────────────────────────────────────────────────────────────────────────────
            // Step 1: Instantiate the manager with the dependencies it needs to inspect settings, log, and profile data.
            var mgr = new SpecimenReturnRulesManager(Logger, BusinessObjectApi, Profile, BusinessRuleSettings, ClientContext);
            // Step 2: Delegate to the manager so it can normalize the shipment according to blueprint rules.
            mgr.PreShip(shipmentRequest, userParams);
            // Step 3: Return control to ShipExec; the request has been adjusted in place by the manager.
        ...
        {
            // ─────────────────────────────────────────────────────────────────────────────
            // Ship Hook — fires on the server during the ship transaction, before/around carrier processing.
            // This override is only used as a backup for pickup association when the client-side flow fails.
            // Blueprint Phase 1 SBR Ship: if the Pickup button/save flow did not persist the pickup,
            // create or populate a Pickup object from user/profile data so the shipment can continue.
            // We delegate the full pickup fallback strategy to a manager class to keep this hook minimal.
            // ─────────────────────────────────────────────────────────────────────────────
            // Step 1: Instantiate the manager with the same ShipExec context used by other hooks.
            var mgr = new SpecimenReturnRulesManager(Logger, BusinessObjectApi, Profile, BusinessRuleSettings, ClientContext);
            // Step 2: Ask the manager to either build a backup pickup or return null to let normal shipping continue.
