...
namespace PSI.Sox
{
    ...
    public class SoxBusinessRules : IBusinessObject
    {
        ...
        public void PreShip(ShipmentRequest shipmentRequest, SerializableDictionary userParams)
        {
            ...
            Logger?.Info("SBR PreShip started.");
            
            try
            {
            if (shipmentRequest.PackageDefaults == null)
            shipmentRequest.PackageDefaults = new PackageDefaultsRequest();
            
            if (shipmentRequest.PackageDefaults.Consignee == null)
            shipmentRequest.PackageDefaults.Consignee = new NameAddress();
            
            if (shipmentRequest.PackageDefaults.ReturnAddress == null && shipmentRequest.PackageDefaults.Shipper != null)
            {
            shipmentRequest.PackageDefaults.ReturnAddress = shipmentRequest.PackageDefaults.Shipper;
            }
            
            bool biologicalSample = string.Equals(shipmentRequest.PackageDefaults.MiscReference4, "true", StringComparison.OrdinalIgnoreCase) ||
            ...
            string shipperCountry = shipmentRequest.PackageDefaults.ReturnAddress?.Country ?? shipmentRequest.PackageDefaults.Consignee?.Country;
            string consigneeCountry = shipmentRequest.PackageDefaults.Consignee?.Country;
            bool isDomesticUS = IsUs(shipperCountry) && IsUs(consigneeCountry);
            bool isCrossBorderOrInternational = !string.IsNullOrWhiteSpace(shipperCountry) && !string.IsNullOrWhiteSpace(consigneeCountry) &&
            !string.Equals(shipperCountry.Trim(), consigneeCountry.Trim(), StringComparison.OrdinalIgnoreCase);
            ...
            Logger?.Info("International return detected; enabled paperless invoice and set export reason to Medical.");
            }
            
            if (biologicalSample)
            {
            SetPackageExtra(shipmentRequest, "RESTRICTED_ARTICLE_TYPE", "32");
            }
            ...
            if (pkg == null)
            {
            pkg = new PackageRequest();
            shipmentRequest.Packages[0] = pkg;
            }
            
            string misc3 = shipmentRequest.PackageDefaults.MiscReference3;
            if (!string.IsNullOrWhiteSpace(misc3))
            {
            if (double.TryParse(misc3, NumberStyles.Any, CultureInfo.InvariantCulture, out double dryIceKg))
            {
            double dryIceLbs = Math.Round(dryIceKg * 2.2046226218d, 2, MidpointRounding.AwayFromZero);
            pkg.Weight = EnsureWeight(pkg.Weight);
            pkg.Weight.Amount = (pkg.Weight.Amount <= 0 ? 0 : pkg.Weight.Amount) + dryIceLbs;
            
            shipmentRequest.PackageDefaults.DryIceWeight = dryIceLbs;
            shipmentRequest.PackageDefaults.DryIcePurpose = "Medical";
            shipmentRequest.PackageDefaults.DryIceWeightUnits = "LBS";
            
            if (isDomesticUS)
            shipmentRequest.PackageDefaults.DryIceRegulationSet = "International Air Transportation Association regulations.";
            else
            shipmentRequest.PackageDefaults.DryIceRegulationSet = "US 49 CFR regulations.";
            
            Logger?.Info($"Applied dry ice weight from MiscReference3: {dryIceKg} KG -> {dryIceLbs} LBS.");
            }
            else
            {
            throw new Exception("Dry Ice Weight (MiscReference3) must be a numeric value in kilograms.");
            }
            }
            
            // Adapter/service selection and service fallback rules.
            // Default to CS adapter services when biological sample is true.
            // These service symbols are expected to be configured in the profile.
            string desiredService = shipmentRequest.PackageDefaults.Service;
            string currentService = shipmentRequest.PackageDefaults.Service;
            
            if (biologicalSample)
            {
            desiredService = ResolveServiceSymbol("CSAdapter Services", currentService);
            }
            
            if (isDomesticUS)
            {
            string ndaEarlyAm = ResolveServiceSymbol("NDA Early AM", desiredService);
            if (ServiceExists(ndaEarlyAm))
            {
            desiredService = ndaEarlyAm;
            }
            else
            {
            string ndaNoSaturday = ResolveServiceSymbol("NDA without Saturday Delivery", desiredService);
            if (ServiceExists(ndaNoSaturday))
            desiredService = ndaNoSaturday;
            }
            }
            else if (isCrossBorderOrInternational)
            {
            string upsExpressSat = ResolveServiceSymbol("UPS Express with Saturday Delivery", desiredService);
            if (ServiceExists(upsExpressSat))
            {
            desiredService = upsExpressSat;
            }
            else
            {
            string saverNoSaturday = ResolveServiceSymbol("UPS Saver without Saturday Delivery", desiredService);
            if (ServiceExists(saverNoSaturday))
            desiredService = saverNoSaturday;
            }
            }
            
            if (!string.IsNullOrWhiteSpace(desiredService))
            shipmentRequest.PackageDefaults.Service = desiredService;
            
            shipmentRequest.PackageDefaults.SaturdayDelivery = !isDomesticUS && shipmentRequest.PackageDefaults.SaturdayDelivery;
            
            Logger?.Info($"SBR PreShip completed. Service={shipmentRequest.PackageDefaults.Service}, BiologicalSample={biologicalSample}, DomesticUS={isDomesticUS}.");
            }
            catch (Exception ex)
            {
            Logger?.Error(ex, "SBR PreShip failed.");
            throw new Exception($"Unable to prepare shipment for shipping: {ex.Message}", ex);
            }
            }
            ...
            c.Equals("United States of America", StringComparison.OrdinalIgnoreCase);
            }
            
            private static PackageWeight EnsureWeight(PackageWeight weight)
            {
            if (weight == null) weight = new PackageWeight();
            return weight;
            }
            
            private void SetPackageExtra(ShipmentRequest shipmentRequest, string key, string value)
            {
            if (shipmentRequest.PackageDefaults == null)
            shipmentRequest.PackageDefaults = new PackageDefaultsRequest();
            
            if (shipmentRequest.PackageDefaults.PackageExtras == null)
            shipmentRequest.PackageDefaults.PackageExtras = new List<KeyValuePair<string, string>>();
            
            var existing = shipmentRequest.PackageDefaults.PackageExtras.FirstOrDefault(x => string.Equals(x.Key, key, StringComparison.OrdinalIgnoreCase));
            if (!string.IsNullOrEmpty(existing.Key))
            {
            shipmentRequest.PackageDefaults.PackageExtras.Remove(existing);
            }
            shipmentRequest.PackageDefaults.PackageExtras.Add(new KeyValuePair<string, string>(key, value));
            }
            
            private string ResolveServiceSymbol(string preferredDisplayName, string fallback)
            {
            try
            {
            if (Profile?.Shippers != null)
            {
            foreach (var shipper in Profile.Shippers)
            {
            if (shipper?.Services == null) continue;
            var svc = shipper.Services.FirstOrDefault(s =>
            s != null &&
            ((s.Description != null && s.Description.Equals(preferredDisplayName, StringComparison.OrdinalIgnoreCase)) ||
            (s.Symbol != null && s.Symbol.Equals(preferredDisplayName, StringComparison.OrdinalIgnoreCase))));
            if (svc != null && !string.IsNullOrWhiteSpace(svc.Symbol))
            return svc.Symbol;
            }
            }
            }
            ...
            private bool ServiceExists(string serviceSymbol)
            {
            if (string.IsNullOrWhiteSpace(serviceSymbol) || Profile?.Shippers == null)
            return !string.IsNullOrWhiteSpace(serviceSymbol);
            
            return Profile.Shippers.Any(shipper => shipper?.Services != null && shipper.Services.Any(s => s != null && string.Equals(s.Symbol, serviceSymbol, StringComparison.OrdinalIgnoreCase)));
            }
        }
        ...
        public ShipmentResponse Ship(ShipmentRequest shipmentRequest, Pickup pickup, bool shipWithoutTransaction, bool print, SerializableDictionary userParams)
        {
            public ShipmentResponse Ship(ShipmentRequest shipmentRequest, Pickup pickup, bool shipWithoutTransaction, bool print, SerializableDictionary userParams)
            {
            Logger?.Info("SBR Ship started.");
            
            try
            {
            // Backup strategy: if pickup association failed on the client, create or hydrate a pickup here.
            if (pickup == null)
            {
            pickup = BuildPickupFromUserProfile(shipmentRequest);
            Logger?.Info("No pickup object supplied from client; constructed backup pickup object from user/profile data.");
            }
            
            // Return null to allow normal ShipExec shipping to continue.
            // We only create/adjust the pickup object as backup support.
            return null;
            }
            catch (Exception ex)
            {
            Logger?.Error(ex, "SBR Ship failed.");
            throw new Exception($"Unable to prepare backup pickup information: {ex.Message}", ex);
            }
            }
            
            private Pickup BuildPickupFromUserProfile(ShipmentRequest shipmentRequest)
            {
            ...
            if (userInfo != null)
            {
            pickup.Company = GetCustomValue(userInfo.CustomData, "Company") ?? userInfo.Company;
            pickup.Contact = GetCustomValue(userInfo.CustomData, "Contact") ?? userInfo.UserName;
            pickup.Phone = GetCustomValue(userInfo.CustomData, "Phone") ?? userInfo.Phone;
            pickup.Address1 = GetCustomValue(userInfo.CustomData, "Address1");
            pickup.Address2 = GetCustomValue(userInfo.CustomData, "Address2");
            pickup.City = GetCustomValue(userInfo.CustomData, "City");
            pickup.StateProvince = GetCustomValue(userInfo.CustomData, "StateProvince");
            pickup.PostalCode = GetCustomValue(userInfo.CustomData, "PostalCode");
            pickup.Country = GetCustomValue(userInfo.CustomData, "Country") ?? shipmentRequest?.PackageDefaults?.ReturnAddress?.Country;
            }
            
            pickup.Reference1 = shipmentRequest?.PackageDefaults?.ShipperReference;
            pickup.Reference2 = shipmentRequest?.PackageDefaults?.MiscReference1;
            pickup.Reference3 = shipmentRequest?.PackageDefaults?.MiscReference2;
            pickup.Reference4 = shipmentRequest?.PackageDefaults?.MiscReference4;
            }
            catch (Exception ex)
            {
            Logger?.Warning($"Pickup backup construction encountered a non-fatal issue: {ex.Message}");
            }
            
            return pickup;
            }
            
            private string GetCustomValue(IEnumerable<CustomDataItem> customData, string key)
            {
            if (customData == null || string.IsNullOrWhiteSpace(key)) return null;
            var item = customData.FirstOrDefault(x => x != null && string.Equals(x.Key, key, StringComparison.OrdinalIgnoreCase));
            return item?.Value;
            }
        }
        ...
        {
            //string dimensionsFileLocation = BusinessRuleSettings[0].Value;

            //bool result = BusinessRuleSettings.ForEach(i => i.Key = )

            //foreach (var connectionString in BusinessRuleSettings)
            //{
            ...
            //{
            //    throw new Exception("ERROR... Could not find the dimenstion file location, please make sur eyou have set the path and file name in commander under Database called daktronicsdimsfile.");
            //}
        ...
        {
            // PLEASE NOTE: The below code will parse a text file by ANY delimiter you set in the 
            // Tools.FromCsv you will need to parse and set your feilds.
            // Tools Tools = new Tools();
            // List<CommodityContent> commodites = File.ReadAllLines("C:\\goods\\Example_Goods.csv")
            //                               .Skip(1)    // <-- Skip header row
            //                               .Select(v => Tools.FromCsv(v))
            //                               .ToList();
