...
namespace PSI.Sox
{
    ...
    public class SoxBusinessRules : IBusinessObject
    {
        ...
        {
            // ──────────────────────────────────────────────────────────────────
            // PreShip Hook — fires on the server immediately before ShipExec sends the shipment to the carrier.
            // This is the authoritative enforcement point for the Marken Phase 1 return-label workflow.
            // Blueprint requirement: international paperless invoice/export reason, biological sample handling,
            // dry ice conversion/application, and service validation/swap rules based on route and adapter needs.
            // We delegate all business decisions to a manager so SoxBusinessRules stays thin and maintainable.
            // ──────────────────────────────────────────────────────────────────
            // Step 1: Create the Marken return shipment manager with all dependencies it needs for rules and logging.
            var mgr = new ReturnShipmentRulesManager(Logger, BusinessObjectApi, Profile, BusinessRuleSettings, ClientContext);
            // Step 2: Delegate the full PreShip workflow so the manager can normalize addresses, validate routing,
            //         apply paperless invoice settings, map dry ice, and adjust services before the carrier call.
        ...
        {
            // ──────────────────────────────────────────────────────────────────
            // Ship Hook — fires on the server during the ship transaction, after PreShip and before the default carrier call.
            // Blueprint requirement: use this only as a backup strategy if client-side pickup association fails.
            // The goal is to create or attach a Pickup object using user/profile defaults so the return label can ship cleanly.
            // We keep the hook thin and delegate to a manager to encapsulate pickup creation and field mapping rules.
            // ──────────────────────────────────────────────────────────────────
            // Step 1: Create the pickup association manager with the runtime dependencies it needs for defaults and logging.
            var mgr = new PickupAssociationManager(Logger, BusinessObjectApi, Profile, BusinessRuleSettings, ClientContext);
            // Step 2: Delegate to the manager; it may return a Pickup to attach or null to let ShipExec continue normally.
