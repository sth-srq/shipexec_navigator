...
namespace PSI.Sox
{
    ...
    public class SoxBusinessRules : IBusinessObject
    {
        ...
        public void PreShip(ShipmentRequest shipmentRequest, SerializableDictionary userParams)
        {
            ...
            if (defaults == null)
            throw new Exception("Shipment package defaults are missing.");
            
            var packageRequest = (shipmentRequest.Packages != null && shipmentRequest.Packages.Count > 0)
            ? shipmentRequest.Packages[0]
            : null;
            
            string shipperCountry = defaults.Shipper != null && !string.IsNullOrWhiteSpace(defaults.Shipper.Country)
            ? defaults.Shipper.Country.Trim().ToUpperInvariant()
            : string.Empty;
            string consigneeCountry = defaults.Consignee != null && !string.IsNullOrWhiteSpace(defaults.Consignee.Country)
            ? defaults.Consignee.Country.Trim().ToUpperInvariant()
            : string.Empty;
            
            bool hasCountries = !string.IsNullOrWhiteSpace(shipperCountry) && !string.IsNullOrWhiteSpace(consigneeCountry);
            bool isDomesticUS = shipperCountry == "US" && consigneeCountry == "US";
            bool isCanadaToCanada = shipperCountry == "CA" && consigneeCountry == "CA";
            bool isCrossBorderOrInternational = hasCountries && shipperCountry != consigneeCountry;
            
            bool biologicalSample = false;
            if (defaults.MiscReference4 != null)
            {
            string misc4 = defaults.MiscReference4.ToString();
            biologicalSample = misc4.Equals("true", StringComparison.OrdinalIgnoreCase) || misc4 == "1" || misc4.Equals("yes", StringComparison.OrdinalIgnoreCase) || misc4.Equals("on", StringComparison.OrdinalIgnoreCase);
            }
            
            if (isCrossBorderOrInternational && packageRequest != null)
            {
            packageRequest.CommercialInvoiceMethod = 1;
            packageRequest.ExportReason = "Medical";
            Logger?.Info("SBR PreShip: enabled paperless invoice and set export reason to Medical for international return.");
            }
            
            if (biologicalSample && packageRequest != null)
            {
            if (packageRequest.PackageExtras == null)
            packageRequest.PackageExtras = new List<PackageExtra>();
            
            packageRequest.PackageExtras.RemoveAll(x => x != null && string.Equals(x.Name, "RESTRICTED_ARTICLE_TYPE", StringComparison.OrdinalIgnoreCase));
            packageRequest.PackageExtras.Add(new PackageExtra { Name = "RESTRICTED_ARTICLE_TYPE", Value = "32" });
            Logger?.Info("SBR PreShip: set biological sample package extra RESTRICTED_ARTICLE_TYPE=32.");
            }
            
            if (packageRequest != null && defaults.MiscReference3 != null && !string.IsNullOrWhiteSpace(defaults.MiscReference3.ToString()))
            {
            decimal dryIceKg;
            if (!decimal.TryParse(defaults.MiscReference3.ToString(), out dryIceKg))
            throw new Exception("Dry Ice Weight (MiscReference3) must be numeric.");
            
            decimal dryIceLbs = dryIceKg * 2.20462m;
            
            packageRequest.DryIceWeight = dryIceLbs;
            packageRequest.DryIcePurpose = "Medical";
            packageRequest.DryIceRegulationSet = isDomesticUS
            ? "International Air Transportation Association regulations."
            : "US 49 CFR regulations.";
            
            if (packageRequest.Weight != null)
            packageRequest.Weight.Amount = packageRequest.Weight.Amount + dryIceLbs;
            else
            packageRequest.Weight = new Weight { Amount = dryIceLbs };
            
            Logger?.Info("SBR PreShip: applied dry ice weight from MiscReference3 (KG converted to LBS) and updated shipment weight.");
            }
            
            if (biologicalSample)
            Logger?.Info("SBR PreShip: biological sample shipment detected; carrier adapter/service selection should use CS Adapter services per environment configuration.");
            
            if (isDomesticUS)
            Logger?.Info("SBR PreShip: domestic US-to-US shipment validation path.");
            else if (isCanadaToCanada)
            Logger?.Info("SBR PreShip: Canada-to-Canada shipment validation path.");
            else if (isCrossBorderOrInternational)
            Logger?.Info("SBR PreShip: cross-border/international shipment validation path.");
            
            Logger?.Info("SBR PreShip: end");
            }
            catch (Exception ex)
            {
        ...
        public ShipmentResponse Ship(ShipmentRequest shipmentRequest, Pickup pickup, bool shipWithoutTransaction, bool print, SerializableDictionary userParams)
        {
            try
            {
            Logger?.Info("SBR Ship: backup pickup strategy not implemented by default.");
