...
namespace PSI.Sox
{
    ...
    public class SoxBusinessRules : IBusinessObject
    {
        ...
        {
            // ──────────────────────────────────────────────────────────────────
            // PreShip Hook — fires on the server immediately before ShipExec calls the carrier.
            // This is the authoritative business-rule checkpoint for the biological returns workflow.
            // Blueprint requirements fulfilled here: international paperless invoice/export reason,
            // biological sample adapter/service decisions, service validation/switching, and dry ice injection.
            // We delegate the business logic to a manager so SoxBusinessRules stays thin and maintainable.
            // ──────────────────────────────────────────────────────────────────
            // Step 1: Instantiate the manager with the server dependencies it needs for logging and settings.
            var mgr = new ReturnShipmentManager(Logger, BusinessObjectApi, BusinessRuleSettings, Profile, ClientContext);
            // Step 2: Delegate all return-shipment enforcement to the manager.
            // The manager will inspect the shipment, apply domestic/international rules, and mutate the request in place.
        ...
        {
            // ──────────────────────────────────────────────────────────────────
            // Ship Hook — fires on the server during the ship operation after PreShip and before the default carrier call.
            // This blueprint uses Ship only as a backup path for pickup association if the client-side pickup workflow fails.
            // The backup logic may create or attach a Pickup object so the shipment can proceed with the return-label workflow.
            // We delegate to a manager to keep the override hook focused on orchestration rather than business details.
            // If the manager returns null, ShipExec continues with its normal carrier processing.
            // ──────────────────────────────────────────────────────────────────
            // Step 1: Instantiate the manager with the same dependencies used for server-side shipping decisions.
            var mgr = new ReturnShipmentManager(Logger, BusinessObjectApi, BusinessRuleSettings, Profile, ClientContext);
            // Step 2: Delegate to the manager's Ship override so it can optionally build or attach a Pickup.
            // Returning null tells ShipExec to continue with its standard shipping flow after the backup logic runs.
