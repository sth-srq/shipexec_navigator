...
namespace PSI.Sox
{
    ...
    public class SoxBusinessRules : IBusinessObject
    {
        ...
        public void PreShip(ShipmentRequest shipmentRequest, SerializableDictionary userParams)
        {
            ...
            {
            Logger?.Info("PreShip: Biological Returns validation and shipment normalization started.");
            ...
            try
            {
            var defaults = shipmentRequest.PackageDefaults ?? new PackageRequest();
            shipmentRequest.PackageDefaults = defaults;
            
            var consigneeCountry = defaults.Consignee != null ? (defaults.Consignee.Country ?? string.Empty).Trim() : string.Empty;
            var shipperCountry = defaults.Shipper != null ? (defaults.Shipper.Country ?? string.Empty).Trim() : string.Empty;
            ...
            bool isInternational = !string.IsNullOrEmpty(consigneeCountry)
            && !string.IsNullOrEmpty(shipperCountry)
            && !string.Equals(consigneeCountry, shipperCountry, StringComparison.OrdinalIgnoreCase);
            
            string biologicalSample = GetPackageValue(shipmentRequest, "MiscReference4");
            bool isBiologicalSample = string.Equals(biologicalSample, "true", StringComparison.OrdinalIgnoreCase)
            || string.Equals(biologicalSample, "1", StringComparison.OrdinalIgnoreCase)
            || string.Equals(biologicalSample, "yes", StringComparison.OrdinalIgnoreCase)
            || string.Equals(biologicalSample, "on", StringComparison.OrdinalIgnoreCase);
            
            string dryIceWeightKgRaw = GetPackageValue(shipmentRequest, "MiscReference3");
            decimal dryIceWeightKg;
            bool hasDryIceWeightKg = decimal.TryParse(dryIceWeightKgRaw, NumberStyles.Any, CultureInfo.InvariantCulture, out dryIceWeightKg);
            
            if (isInternational)
            {
            SetOrCreateCustomData(shipmentRequest, "CommercialInvoiceMethod", "1");
            SetOrCreateCustomData(shipmentRequest, "ExportReason", "Medical");
            Logger?.Info("PreShip: Enabled paperless invoice and export reason Medical for international shipment.");
            }
            
            if (isBiologicalSample)
            {
            SetOrCreateCustomData(shipmentRequest, "PackageExtras.RESTRICTED_ARTICLE_TYPE", "32");
            Logger?.Info("PreShip: Applied biological sample package extra RESTRICTED_ARTICLE_TYPE=32.");
            }
            
            if (hasDryIceWeightKg)
            {
            decimal dryIceWeightLbs = dryIceWeightKg * 2.2046226218m;
            AddOrUpdateShipmentAmount(shipmentRequest, "DryIceWeight", dryIceWeightLbs.ToString("0.####", CultureInfo.InvariantCulture));
            SetOrCreateCustomData(shipmentRequest, "DryIcePurpose", "Medical");
            
            if (isDomesticUS)
            SetOrCreateCustomData(shipmentRequest, "DryIceRegulationSet", "International Air Transportation Association regulations.");
            else
            SetOrCreateCustomData(shipmentRequest, "DryIceRegulationSet", "US 49 CFR regulations.");
            ...
            if (pkg.Weight != null)
            {
            decimal.TryParse(pkg.Weight.ToString(), NumberStyles.Any, CultureInfo.InvariantCulture, out currentWeight);
            }
            currentWeight += dryIceWeightLbs;
            SetPackageWeight(pkg, currentWeight);
            }
            }
            
            Logger?.Info($"PreShip: Dry ice weight applied. KG={dryIceWeightKg}, LBS={dryIceWeightLbs}.");
            }
            
            string currentService = GetServiceSymbol(shipmentRequest);
            string selectedService = currentService;
            
            if (isBiologicalSample)
            {
            selectedService = ResolveBiologicalSampleService(shipmentRequest, currentService, consigneeCountry, shipperCountry);
            if (!string.IsNullOrEmpty(selectedService) && !string.Equals(selectedService, currentService, StringComparison.OrdinalIgnoreCase))
            {
            SetServiceSymbol(shipmentRequest, selectedService);
            Logger?.Info($"PreShip: Service overridden for biological sample. {currentService} -> {selectedService}");
            }
            }
            ...
            {
            Logger?.Error("PreShip failed.", ex);
            throw new Exception("Unable to prepare the shipment for biological returns processing. " + ex.Message, ex);
            }
            }
        }
        ...
        public ShipmentResponse Ship(ShipmentRequest shipmentRequest, Pickup pickup, bool shipWithoutTransaction, bool print, SerializableDictionary userParams)
        {
            public ShipmentResponse Ship(ShipmentRequest shipmentRequest, Pickup pickup, bool shipWithoutTransaction, bool print, SerializableDictionary userParams)
            {
            Logger?.Info("Ship: custom backup hook entered.");
            
            try
            {
            // Primary blueprint logic relies on client-side pickup association.
            // If a custom Pickup object strategy is implemented later, it can be added here.
            // Returning null preserves default ShipExec carrier processing.
            return null;
            }
            catch (Exception ex)
            {
            Logger?.Error("Ship override failed.", ex);
            throw new Exception("Unable to process shipment. " + ex.Message, ex);
            }
            }
        }
