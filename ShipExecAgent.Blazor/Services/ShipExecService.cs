...
using ShipExecAgent.BusinessLogic.RequestGeneration;
using ShipExecAgent.DAL.Managers;
using ShipExecAgent.Helpers;
using ShipExecAgent.Shared.Helpers;
using ShipExecAgent.Shared.Interfaces;
using ShipExecAgent.Shared.Models;
using CompanyInfo = ShipExecAgent.Shared.Models.CompanyInfo;
using DalVariance = ShipExecAgent.DAL.Entities.Variance;
using DalTemplate = ShipExecAgent.DAL.Entities.CompanyTemplate;
    ...
    ILogger<ShipExecService> logger) : IShipExecService
{
    ...
    public Task<string> ExportShippersCsvAsync()
    {
        ...
        return Task.Run(() =>
        {
            ...
            var clientLogic = ShipExecAgent.ClientSpecificLogic.ClientLogicResolver.Resolve(GetCurrentCompany()?.Name);
    ...
    {
        var tempManager = new ShipExecAgent.BusinessLogic.AppManager(jwtJson, adminUrl);
        return Task.Run(() =>
    ...
    public async Task<string> ExportAllCompanyAssetsAsync(string outputFolder)
    {
        var appMgr = GetAppManagerOrThrow();
        var companies = _lastCompanies;
        if (companies.Count == 0)
            return "No companies loaded.";

        var report = new System.Text.StringBuilder();
        report.AppendLine($"Export started — {companies.Count} companies, output: {outputFolder}");
        report.AppendLine();
        ...
        {
            var companyFolder = Path.Combine(outputFolder, SanitizeFileName(company.Name));
            Directory.CreateDirectory(companyFolder);
            report.AppendLine($"═══ {company.DisplayName} ═══");

            // ── CBRs with profile association ────────────────────────────────
            try
            {
                await Task.Run(() => appMgr.SetCompany(company.Id, company.Name));
                var cbrResults = await Task.Run(() => appMgr.GetClientBusinessRulesWithProfilesForCompany());

                int cbrFileCount = 0;
                foreach (var (rule, profileNames) in cbrResults)
                {
                    if (profileNames.Count == 0) continue;

                    foreach (var profileName in profileNames)
                    {
                        var trimmedProfile = profileName.Replace(" ", "");
                        var fileName = SanitizeFileName($"{company.Name}{trimmedProfile}{rule.Name}") + ".js";
                        var filePath = Path.Combine(companyFolder, fileName);
                        await File.WriteAllTextAsync(filePath, rule.Script ?? string.Empty);
                        cbrFileCount++;
                    }
                }
                report.AppendLine($"  CBRs: {cbrResults.Count} rules → {cbrFileCount} files saved");
            }
            catch (Exception ex)
            {
                report.AppendLine($"  CBRs: ERROR — {ex.Message}");
            }

            // ── CustomTemplates ──────────────────────────────────────────────
            try
            {
                var templates = await Task.Run(() => appMgr.GetTemplatesForCompany(company.Id));

                // Templates don't have profile association; check if profiles reference them
                // by fetching profiles and matching template IDs
                var profiles = await Task.Run(() => appMgr.GetProfiles());

                int templateFileCount = 0;
                foreach (var template in templates)
                {
                    // Find profiles that use this template (via full profile fetch checking)
                    // Since templates are company-level, save with company name
                    var fileName = SanitizeFileName(
                        $"{company.Name}_{template.TemplateType}_{template.TemplateName}") + ".html";
                    var filePath = Path.Combine(companyFolder, fileName);
                    await File.WriteAllTextAsync(filePath, template.TemplateHtml ?? string.Empty);
                    templateFileCount++;
                }
                report.AppendLine($"  Templates: {templates.Count} → {templateFileCount} files saved");
            }
            catch (Exception ex)
            {
                report.AppendLine($"  Templates: ERROR — {ex.Message}");
            }
        ...
        try
        {
            await Task.Run(() => appMgr.SetCompany(_currentCompanyId, string.Empty));
        }
        catch { /* best effort */ }

        report.AppendLine($"Export complete.");
